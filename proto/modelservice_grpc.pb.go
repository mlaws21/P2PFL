...
const (
	...
	ModelService_GetBootModel_FullMethodName  = "/modelservice.ModelService/GetBootModel"
)
...
type ModelServiceClient interface {
	...
	GetBootModel(ctx context.Context, in *GetBootModelRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[GetBootModelReply], error)
}
...
func (c *modelServiceClient) GetBootModel(ctx context.Context, in *GetBootModelRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[GetBootModelReply], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &ModelService_ServiceDesc.Streams[1], ModelService_GetBootModel_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[GetBootModelRequest, GetBootModelReply]{ClientStream: stream}
	if err := x.ClientStream.SendMsg(in); err != nil {
		return nil, err
	}
	if err := x.ClientStream.CloseSend(); err != nil {
...
// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type ModelService_GetBootModelClient = grpc.ServerStreamingClient[GetBootModelReply]
...
type ModelServiceServer interface {
	...
	GetBootModel(*GetBootModelRequest, grpc.ServerStreamingServer[GetBootModelReply]) error
...
func (UnimplementedModelServiceServer) GetBootModel(*GetBootModelRequest, grpc.ServerStreamingServer[GetBootModelReply]) error {
	return status.Errorf(codes.Unimplemented, "method GetBootModel not implemented")
}
...
func _ModelService_GetBootModel_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(GetBootModelRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(ModelServiceServer).GetBootModel(m, &grpc.GenericServerStream[GetBootModelRequest, GetBootModelReply]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type ModelService_GetBootModelServer = grpc.ServerStreamingServer[GetBootModelReply]
...
var ModelService_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		{
			StreamName:    "GetBootModel",
			Handler:       _ModelService_GetBootModel_Handler,
			ServerStreams: true,
		},
	},
